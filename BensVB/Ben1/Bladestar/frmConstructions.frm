...
End Select
For i = 1 To 2
If RandomWeapons = True Then StartWeapon(i) = Int(Rnd * WeaponDataCount + 1)
Next i
...
End Select
For i = 1 To 2
lblWeaponName(i).Caption = WeaponData(Player(i).Weapon(1)).Name
lblWeapon2Name(i).Caption = WeaponData(Player(i).Weapon(2)).Name
lblAmmo(i).Caption = Player(i).Ammo(1)
lblClipAmmo(i).Caption = Player(i).ClipAmmo(1)
Next i
Call UpdateSpawns
Terminated = False
Call GameLoop
...
If VisualContact = True Then
    ...
    If Distance(Player(1).XCoord, Player(1).YCoord, Player(2).XCoord, Player(2).YCoord) <= WeaponData(Player(i).Weapon(1)).ShotSpeed * WeaponData(Player(i).Weapon(1)).ShotLifespan Then
...
For j = ExplosionMin To ExplosionCount
    If Explosion(j).Deleted = False Then
        BitBlt BackBuffDC(i), picScreen(i).ScaleWidth / 2 + Explosion(j).XCoord - 40 - Player(i).XCoord, picScreen(i).ScaleHeight / 2 + Explosion(j).YCoord - 40 - Player(i).YCoord, 80, 80, ExplosionMaskDC(Explosion(j).GraphicsIndex), 0, 0, vbMergePaint
        BitBlt BackBuffDC(i), picScreen(i).ScaleWidth / 2 + Explosion(j).XCoord - 40 - Player(i).XCoord, picScreen(i).ScaleHeight / 2 + Explosion(j).YCoord - 40 - Player(i).YCoord, 80, 80, ExplosionGraphicsDC(Explosion(j).GraphicsIndex), 0, 0, vbSrcAnd
    End If
Next j
'>>> shots
