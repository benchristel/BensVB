...
If WeaponData(Player(1).Weapon).SemiAuto = True And Player(1).Firing = False And Player(1).CoolDownTime <= 0 Then
If Distance(Player(1).XCoord, Player(1).YCoord, Player(2).XCoord, Player(2).YCoord) > 40 Or WeaponData(Player(1).Weapon).Melee = False Then
    Call Fire(1)
    Player(1).CoolDownTime = WeaponData(Player(1).Weapon).CoolDown
End If
End If
    Player(1).Firing = True
    frmMain.lblClipAmmo(1).Caption = Player(1).ClipAmmo
...
If Player(1).PickUpItem = False Then
    ...
    If Player(1).Ammo > 0 Or Player(1).ClipAmmo > 0 Then
        WeaponCount = WeaponCount + 1
        ReDim Preserve Weapon(1 To WeaponCount)
        With Weapon(WeaponCount)
            .Ammo = Player(1).Ammo
            .ClipAmmo = Player(1).ClipAmmo
            .ClipSize = WeaponData(Player(1).Weapon).ClipSize
            .Deleted = False
            .DespawnEnabled = True
            .Name = WeaponData(Player(1).Weapon).Name
            .TimeToDespawn = 40
            .WeaponType = Player(1).Weapon
    ...
    End If
        With Player(1)
            .Ammo = Weapon(i).Ammo
            .ClipAmmo = Weapon(i).ClipAmmo
            .Weapon = Weapon(i).WeaponType
            .Reloading = False
        End With
        frmMain.lblPlayerStatus(1).Caption = ""
        Weapon(i).Deleted = True
        frmMain.lblAmmo(1).Caption = Player(1).Ammo
        frmMain.lblClipAmmo(1).Caption = Player(1).ClipAmmo
        frmMain.lblWeaponName(1).Caption = WeaponData(Player(1).Weapon).Name
...
End If
    ...
    Player(1).ReloadTime = WeaponData(Player(1).Weapon).ReloadTime
    End If
    Player(1).Firing = False
Else
...
If WeaponData(Player(2).Weapon).SemiAuto = True And Player(2).Firing = False And Player(2).CoolDownTime <= 0 Then
If Distance(Player(2).XCoord, Player(2).YCoord, Player(1).XCoord, Player(1).YCoord) > 40 Or WeaponData(Player(2).Weapon).Melee = False Then
    Call Fire(2)
    Player(2).CoolDownTime = WeaponData(Player(2).Weapon).CoolDown
End If
End If
    Player(2).Firing = True
    frmMain.lblClipAmmo(2).Caption = Player(2).ClipAmmo
...
If Player(2).PickUpItem = False Then
    ...
    If Player(2).Ammo > 0 Or Player(2).ClipAmmo > 0 Then
        WeaponCount = WeaponCount + 1
        ReDim Preserve Weapon(1 To WeaponCount)
        With Weapon(WeaponCount)
            .Ammo = Player(2).Ammo
            .ClipAmmo = Player(2).ClipAmmo
            .ClipSize = WeaponData(Player(2).Weapon).ClipSize
            .Deleted = False
            .DespawnEnabled = True
            .Name = WeaponData(Player(2).Weapon).Name
            .TimeToDespawn = 40
            .WeaponType = Player(2).Weapon
    ...
    End If
        ...
        Weapon(i).Deleted = True
        frmMain.lblPlayerStatus(2).Caption = ""
        frmMain.lblAmmo(2).Caption = Player(2).Ammo
        frmMain.lblClipAmmo(2).Caption = Player(2).ClipAmmo
        frmMain.lblWeaponName(2).Caption = WeaponData(Player(2).Weapon).Name
...
End If
    ...
    Player(2).ReloadTime = WeaponData(Player(2).Weapon).ReloadTime
    End If
    Player(2).Firing = False
Else
...
End If
If Player(i).MoveForward = True Then tempslope(1) = FindLine(0, 0, Player(i).TargetX, Player(i).TargetY, 3 + WeaponData(Player(i).Weapon).SpeedBonus)
If Player(i).StrafeLeft = True Then tempslope(2) = FindLine(0, 0, Player(i).TargetY, -Player(i).TargetX, 3 + WeaponData(Player(i).Weapon).SpeedBonus)
If Player(i).StrafeRight = True Then tempslope(3) = FindLine(0, 0, -Player(i).TargetY, Player(i).TargetX, 3 + WeaponData(Player(i).Weapon).SpeedBonus)
If Player(i).MoveBack = True Then tempslope(4) = FindLine(0, 0, -Player(i).TargetX, -Player(i).TargetY, 3 + WeaponData(Player(i).Weapon).SpeedBonus)
With tempslope(5)
...
For j = WeaponMin To WeaponCount
    If Weapon(j).Deleted = False And Weapon(j).WeaponType = Player(i).Weapon And Player(i).Dead = False Then
        If Overlap(Player(i).XCoord, Player(i).YCoord, 0, 0, Weapon(j).XCoord - 20, Weapon(j).YCoord - 20, 40, 40) = True Then
            Player(i).Ammo = Player(i).Ammo + Weapon(j).Ammo + Weapon(j).ClipAmmo
            Weapon(j).Deleted = True
            frmMain.lblAmmo(i).Caption = Player(i).Ammo
        End If
    End If
Next j
'>>>shooting
If Distance(Player(1).XCoord, Player(1).YCoord, Player(2).XCoord, Player(2).YCoord) > 40 Or WeaponData(Player(i).Weapon).Melee = False Then
    'shooting
If Player(i).CoolDownTime <= 0 And Player(i).Firing = True Then
    Player(i).CoolDownTime = WeaponData(Player(i).Weapon).CoolDown
    If WeaponData(Player(i).Weapon).SemiAuto = False Then
    Call Fire(i)
    End If
Else
    Player(i).CoolDownTime = Player(i).CoolDownTime - 1
End If
ElseIf WeaponData(Player(i).Weapon).Melee = True Then
    'melee
If Player(i).CoolDownTime <= 0 And Player(i).Firing = True Then
    ...
    If Player(i).Firing = True Then
        ...
        If Player(2).Dead = False Then
            If WeaponData(Player(1).Weapon).MeleeDamage >= Player(2).Shield Then
                Player(2).HP = Player(2).HP - WeaponData(Player(1).Weapon).MeleeDamage + Player(2).Shield
                Player(2).Shield = 0
                If Player(2).HP <= 0 Then
                    Player(2).HP = 0
                End If
            Else
                Player(2).Shield = Player(2).Shield - WeaponData(Player(1).Weapon).MeleeDamage
            End If
        End If
        Else
        If Player(1).Dead = False Then ' i=2
            If WeaponData(Player(2).Weapon).MeleeDamage >= Player(1).Shield Then
                Player(1).HP = Player(1).HP - WeaponData(Player(2).Weapon).MeleeDamage + Player(1).Shield
                Player(1).Shield = 0
                If Player(1).HP <= 0 Then
                    Player(1).HP = 0
                End If
            Else
                Player(1).Shield = Player(1).Shield - WeaponData(Player(2).Weapon).MeleeDamage
            End If
        End If
        End If
    End If
...
If Player(i).Reloading = True And Player(i).ReloadTime = 0 Then
    If Player(i).Ammo > WeaponData(Player(i).Weapon).ClipSize - Player(i).ClipAmmo Then
        Player(i).Ammo = Player(i).Ammo - WeaponData(Player(i).Weapon).ClipSize + Player(i).ClipAmmo
        Player(i).ClipAmmo = WeaponData(Player(i).Weapon).ClipSize
    Else
        Player(i).ClipAmmo = Player(i).ClipAmmo + Player(i).Ammo
        Player(i).Ammo = 0
    End If
    frmMain.lblPlayerStatus(i).Caption = ""
    frmMain.lblAmmo(i).Caption = Player(i).Ammo
    frmMain.lblClipAmmo(i).Caption = Player(i).ClipAmmo
...
If Player(Index).ClipAmmo > 0 Then
Player(Index).ClipAmmo = Player(Index).ClipAmmo - 1
ShotCount = ShotCount + 1
ReDim Preserve Shot(1 To ShotCount)
xspread = Rnd * (WeaponData(Player(Index).Weapon).ShotSpread * 2) - WeaponData(Player(Index).Weapon).ShotSpread
yspread = Rnd * (WeaponData(Player(Index).Weapon).ShotSpread * 2) - WeaponData(Player(Index).Weapon).ShotSpread
tempslope = FindLine(0, 0, Player(Index).TargetX + xspread, Player(Index).TargetY + yspread, WeaponData(Player(Index).Weapon).ShotSpeed)
With Shot(ShotCount)
    .Alignment = Index
    .Bounce = WeaponData(Player(Index).Weapon).Bounce
    .Damage = WeaponData(Player(Index).Weapon).Damage
    .DamageMultiplier = WeaponData(Player(Index).Weapon).DamageMultiplier
    .Deleted = False
    .Distance = 0
    .Explosive = WeaponData(Player(Index).Weapon).ShotExplosive
    .GraphicsIndex = WeaponData(Player(Index).Weapon).ShotGraphicsIndex
    .Lifespan = WeaponData(Player(Index).Weapon).ShotLifespan
    .Radius = WeaponData(Player(Index).Weapon).ShotRadius
    .Visible = WeaponData(Player(Index).Weapon).ShotVisible
    .Speed = WeaponData(Player(Index).Weapon).ShotSpeed
    .XCoord = Player(Index).XCoord
    .YCoord = Player(Index).YCoord
    .XVector = tempslope.Run
    .YVector = tempslope.Rise
End With
frmMain.lblClipAmmo(Index).Caption = Player(Index).ClipAmmo
End If
End Sub

Public Sub KillPlayer(Index)
Dim i, j
If Player(Index).Ammo > 0 Or Player(Index).ClipAmmo > 0 Then
        WeaponCount = WeaponCount + 1
        ReDim Preserve Weapon(1 To WeaponCount)
        With Weapon(WeaponCount)
            .Ammo = Player(Index).Ammo
            .ClipAmmo = Player(Index).ClipAmmo
            .ClipSize = WeaponData(Player(Index).Weapon).ClipSize
            .Deleted = False
            .Name = WeaponData(Player(Index).Weapon).Name
'            .SemiAuto = WeaponData(Player(Index).Weapon).SemiAuto
            .TimeToDespawn = 35
            .DespawnEnabled = True
            .WeaponType = Player(Index).Weapon
            .XCoord = Player(Index).XCoord
            .YCoord = Player(Index).YCoord
        End With
End If
...
.Weapon = StartWeapon
Else
.Weapon = Int(Rnd * WeaponDataCount + 1)
End If
If WeaponData(Player(Index).Weapon).StartAmmo - WeaponData(Player(Index).Weapon).ClipSize >= 0 Then
    .Ammo = WeaponData(Player(Index).Weapon).StartAmmo - WeaponData(Player(Index).Weapon).ClipSize
    .ClipAmmo = WeaponData(Player(Index).Weapon).ClipSize
Else
    .Ammo = 0
    .ClipAmmo = WeaponData(Player(Index).Weapon).StartAmmo
End If
.CoolDownTime = 0
...
End If
Next i
frmMain.lblAmmo(Index).Caption = Player(Index).Ammo
frmMain.lblClipAmmo(Index).Caption = Player(Index).ClipAmmo
frmMain.lblWeaponName(Index).Caption = WeaponData(Player(Index).Weapon).Name
End Sub
